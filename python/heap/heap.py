...
class DWayHeap(object):
    ...
    def __init__(self, elements: List[Any] = [], priorities: List[float] = [], branching_factor: int = 2) -> None:
        ...
        if len(elements) != len(priorities):
            raise ValueError(f'The length of the elements list ({len(elements)})'
                             f' must match with the length of priorities list ({len(priorities)})')
    ...
    def _validate(self) -> bool:
        ...
        """
        current_index: int = 0
        first_leaf: int = self.first_leaf_index()

        while current_index < first_leaf:
            current_priority: float = self._pairs[current_index][0]
            first_child: int = self._first_child_index(current_index)
            last_child_guard: int = min(first_child + self.D, len(self))
    ...
    def _push_down(self, index: int) -> None:
        ...
        input_priority: float = input_pair[0]
        current_index: int = index

        first_leaf: int = self.first_leaf_index()

        while current_index < first_leaf:
            child_index: int = self._highest_priority_child_index(current_index)
    ...
    def _bubble_up(self, index: int) -> None:
        ...
        while index > 0:
            parent_index: int = self._parent_index(index)
    ...
    def _highest_priority_child_index(self, index: int) -> Optional[int]:
        ...
        """
        first_index: int = self._first_child_index(index)
        size: int = len(self)
        last_index: int = min(first_index + self.D, size)
    ...
    def _heapify(self, elements: List[Any], priorities: List[float]) -> None:
       ...
       """
        ...
        last_inner_node_index: int = self.first_leaf_index() - 1
    ...
    def top(self) -> Any:
        ...
        """
        if self.is_empty():
            raise RuntimeError('Method top called onn empty heap')
    ...
    def peek(self) -> any:
        ...
        """
        if self.is_empty():
            raise RuntimeError('method peek called on empty heap')
    ...
    def print(self):
        ...
        for i in range(len(self._pairs)):
            print('{} -> {}'.format(self._pairs[i][1], self._pairs[i][0]))
